...
namespace AnnotationsProject {
	public class Parser {
					...
					switch (membAnn.AnnotationName) {
						...
						case "Column":
							if (!basicTypes.Contains(membAnn.FieldType))
								throw new Exception($"Use annotation @{membAnn.AnnotationName} over a basic type only.");
							break;
						case "Many2One":
							if (basicTypes.Contains(membAnn.FieldType))
							...
							if (membAnn.FieldType != membAnn.Attributes["target"] ||
								...
								throw new Exception("Incorrect value for attribute \"target\" of annotation @Many2One.");
							break;
						case "One2Many":
							...
							if (targetAnnotations.Count() == 0)
								throw new Exception("Misuse of annotation @One2Many.");
							...
							if (elementsType != membAnn.Attributes["target"] ||
								...
								throw new Exception("Incorrect values for attributes of annotation @One2Many.");
							break;
					}
		...
		private TableAnnotation sdec() {
			...
			var tableAnnotation = new TableAnnotation("Table", interfaceModifier, interfaceName);
		...
		private string table() {
			Expect(Token.AT, Token.TABLE, Token.ROUNDOPEN, Token.NAME, Token.EQUALS, Token.QUOTEDLITERAL);
		...
		private MemberAnnotation idmemb() {
			Expect(Token.AT, Token.ID, Token.ROUNDOPEN, Token.NAME, Token.EQUALS, Token.QUOTEDLITERAL);
			string nameAttribute = lexan.TokenValue;
			Expect(Token.ROUNDCLOSE);
			...
			var memberAnnotation = new MemberAnnotation("Id", fieldType, fieldName);
		...
		private MemberAnnotation colmemb() {
			Expect(Token.COLUMN, Token.ROUNDOPEN, Token.NAME, Token.EQUALS, Token.QUOTEDLITERAL);
			string nameAttribute = lexan.TokenValue;
			string lengthAttribute = null;
			if (lexan.Lookahead != Token.ROUNDCLOSE) {
				Expect(Token.COMMA, Token.WHITESPACE, Token.LENGTH, Token.EQUALS, Token.QUOTEDLITERAL);
				int numb = 0;
			...
			var memberAnnotation = new MemberAnnotation("Column", fieldType, fieldName);
			memberAnnotation.Attributes.Add("name", nameAttribute);
		...
		private MemberAnnotation relmemb() {
			...
			if (relType == "One2Many") {
				Expect(Token.COMMA, Token.WHITESPACE, Token.MAPPEDBY, Token.EQUALS, Token.QUOTEDLITERAL);
			...
			var memberAnnotation = new MemberAnnotation(relType, fieldType, fieldName);
			memberAnnotation.Attributes.Add("name", nameAttribute);
