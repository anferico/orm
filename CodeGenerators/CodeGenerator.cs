...
namespace AnnotationsProject {
	public class CodeGenerator {
		...
		Dictionary<string, string> langToSQL = new Dictionary<string, string>() {
			{"Integer", "INT"}, {"Float", "FLOAT"}, {"Double", "DOUBLE PRECISION"},
			{"Boolean", "BOOLEAN"}, {"Char", "CHARACTER"}, {"String", "VARCHAR"}
		};
		...
		public void GeneratePlainCode() {
			foreach (var tabAnn in tableAnnotations) {
				byte[] classCode = Encoding.ASCII.GetBytes(GenerateInterfaceCode(tabAnn));
				File.Create($"../../{tabAnn.InterfaceName}.cs").Write(classCode, 0, classCode.Length);
			}
		}

		private string GenerateInterfaceCode(TableAnnotation tabAnn) {
			var strBuilder = new StringBuilder();
			strBuilder.Append($"using System.Collections.Generic; namespace AnnotationsProject {{" +
		...
		public void GenerateDatabaseSchema() {
			foreach (var tabAnn in tableAnnotations) {
				byte[] sqlCode = Encoding.ASCII.GetBytes(GenerateSQLCode(tabAnn));
				File.Create($"../../{tabAnn.Attributes["name"]}.sql").Write(sqlCode, 0, sqlCode.Length);
			}
		}
